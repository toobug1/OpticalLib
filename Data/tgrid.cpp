...
Math::Vector2 TGrid::get_x_value_i(unsigned int n1, unsigned int n2) const
{
    return _origin + _step.mul( Math::Vector2((double)n1, (double)n2) );
}

double TGrid::get_y_value(unsigned int n1, unsigned int n2) const
{
    assert(n1 < _size[0]);
    assert(n2 < _size[1]);
...
double & TGrid::get_y_value(unsigned int n1, unsigned int n2)
{
    assert(n1 < _size[0]);
    assert(n2 < _size[1]);
...
const Math::Vector2 & TGrid::get_d_value(unsigned int n1, unsigned int n2) const
{
...
Math::Vector2 & TGrid::get_d_value(unsigned int n1, unsigned int n2)
{
    assert(_interpolation == BicubicDeriv);
    assert(n1 < _size[0]);
    assert(n2 < _size[1]);
...
double TGrid::get_nearest_y(const Math::Vector2 & v) const
{
    unsigned int x[2];
...
const Math::Vector2 & TGrid::get_nearest_d(const Math::Vector2 & v) const
{
    assert(_interpolation == BicubicDeriv);
...
double & TGrid::get_nearest_y(const Math::Vector2 & v)
{
    unsigned int x[2];
...
Math::Vector2 & TGrid::get_nearest_d(const Math::Vector2 & v)
{
    assert(_interpolation == BicubicDeriv);
...
double TGrid::interpolate(const Math::Vector2 & v) const
{
    unsigned int x[2];
    (this->*_lookup)(x, v);
...
Math::Vector2 TGrid::interpolate_deriv(const Math::Vector2 & v) const
{
    Math::Vector2 res;
    unsigned int x[2];
    (this->*_lookup)(x, v);
...
void TGrid::set_metrics(const Math::Vector2 & origin, const Math::Vector2 & step)
{
    invalidate();
    _origin = origin;
    _step = step;
...
const Math::Vector2 & TGrid::get_origin() const
{
    return _origin;
}

const Math::Vector2 & TGrid::get_step() const
{
    return _step;
}

void TGrid::resize(unsigned int n1, unsigned int n2)
{
    (this->*_resize)(n1, n2);
    _size[0] = n1;
    _size[1] = n2;
...
unsigned int TGrid::get_dimensions() const
{
    return 2;
}

unsigned int TGrid::get_count(unsigned int dimension) const
{
    return _size[dimension];
}

void TGrid::invalidate()
{
    _lookup = _update;
}

TGrid::TGrid(unsigned int n1, unsigned int n2,
    ...
    : TSet(),
      ...
      _d_data(),
      _poly(),
      _update(&TGrid::update_linear),
      _lookup(&TGrid::update_linear),
      _resize(&TGrid::resize_y),
      _origin(origin),
      _step(step)
{
    _origin = origin;
...
void TGrid::set_all_y(double y)
{
    GOPTICAL_FOREACH(i, _y_data)
            *i = y;
}

void TGrid::set_all_d(const Math::Vector2 & deriv)
{
    GOPTICAL_FOREACH(i, _d_data)
            *i = deriv;
}

double TGrid::get_x_value(unsigned int x, unsigned int dimension) const
{
    assert(dimension < 2);
    return _origin[dimension] + _step[dimension] * x;
}

double TGrid::get_y_value(const unsigned int n[]) const
{
    return get_y_value(n[0], n[1]);
}

double TGrid::interpolate(const double x[]) const
{
    return interpolate(Math::Vector2(x[0], x[1]));
}

double TGrid::interpolate(const double x[], unsigned int deriv, unsigned int dimension) const
{
    switch (deriv)
    {
    case 0:
...
Math::range_t TGrid::get_x_range(unsigned int d) const
{
...
void TGrid::set_interpolation(Interpolation i)
{
    switch (i)
    {
    case Nearest:
        _update = &TGrid::update_nearest;
        _resize = &TGrid::resize_y;
        _d_data.clear();
        _poly.clear();
        break;
    ...
    case Bicubic:
        _update = &TGrid::update_bicubic;
        _resize = &TGrid::resize_y;
        _d_data.clear();
        break;

    case BicubicDiff:
        _update = &TGrid::update_bicubic_diff;
        _resize = &TGrid::resize_y;
        _d_data.clear();
        break;

    case BicubicDeriv:
        _update = &TGrid::update_bicubic_deriv;
        _resize = &TGrid::resize_yd;
...
void TGrid::update_nearest(unsigned int x[2], const Math::Vector2 & v) const
{
    TGrid * this_ = const_cast<TGrid *>(this);

    if (_size[0] < 1 || _size[1] < 1)
        throw Error("data set doesn't contains enough data");

    this_->_lookup = &TGrid::lookup_nearest;
    this_->_interpolate_y = &TGrid::interpolate_nearest_y;
    this_->_interpolate_d = &TGrid::interpolate_nearest_d;

    return lookup_nearest(x, v);
}

double TGrid::interpolate_nearest_y(const unsigned int x[2], const Math::Vector2 & v) const
{
    return _y_data[x[0] + x[1] * _size[0]];
}

void TGrid::interpolate_nearest_d(const unsigned int x[2], Math::Vector2 & d, const Math::Vector2 & v) const
{
    d.set(0);
}
...
void TGrid::update_linear(unsigned int x[2], const Math::Vector2 & v) const
{
    TGrid * this_ = const_cast<TGrid *>(this);
    ...
    this_->_lookup = &TGrid::lookup_interval;
    this_->_interpolate_y = &TGrid::interpolate_linear_y;
    this_->_interpolate_d = &TGrid::interpolate_linear_d;

    return lookup_interval(x, v);
}

double TGrid::interpolate_linear_y(const unsigned int x[2], const Math::Vector2 & v) const
{
...
void TGrid::interpolate_linear_d(const unsigned int x[2], Math::Vector2 & d, const Math::Vector2 & v) const
{
    unsigned int s = _size[0];
    unsigned int idx = x[0] + s * x[1];
...
void TGrid::set_poly(poly_t &p, const double t[16])
{
    /*
       0    1
    0 d00  d01
...
void TGrid::get_cross_deriv_diff(double cd[]) const
{
    unsigned int x0, x1;
...
void TGrid::get_deriv_diff(Math::Vector2 d[]) const
{
    unsigned int x0, x1;
...
template <int v>
void TGrid::get_deriv_smooth(Math::Vector2 d[], unsigned int w, unsigned int o) const
{
...
void TGrid::update_bicubic(unsigned int x[2], const Math::Vector2 & v) const
{
    TGrid * this_ = const_cast<TGrid *>(this);
    ...
    this_->_lookup = &TGrid::lookup_interval;
    this_->_interpolate_y = &TGrid::interpolate_bicubic_y;
    this_->_interpolate_d = &TGrid::interpolate_bicubic_d;

    return lookup_interval(x, v);
}

void TGrid::update_bicubic_diff(unsigned int x[2], const Math::Vector2 & v) const
{
    TGrid * this_ = const_cast<TGrid *>(this);
...
void TGrid::update_bicubic_deriv(unsigned int x[2], const Math::Vector2 & v) const
{
    TGrid * this_ = const_cast<TGrid *>(this);
...
double TGrid::interpolate_bicubic_y(const unsigned int x[2], const Math::Vector2 & v) const
{
...
void TGrid::interpolate_bicubic_d(const unsigned int x[2], Math::Vector2 & d, const Math::Vector2 & v) const
{
    const poly_t &p = _poly[x[0] + x[1] * (_size[0] - 1)];
    Math::Vector2 t((v - _origin) / _step - Math::Vector2((double)x[0], (double)x[1]));
...
void TGrid::lookup_nearest(unsigned int x[2], const Math::Vector2 & v) const
{
...
void TGrid::lookup_interval(unsigned int x[2], const Math::Vector2 & v) const
{
...
void TGrid::resize_y(unsigned int x1, unsigned int x2)
{
    invalidate();
    assert(x1 >= 2 && x2 >=2);
...
void TGrid::resize_yd(unsigned int x1, unsigned int x2)
{
    invalidate();
    assert(x1 >= 2 && x2 >=2);
    _y_data.resize(x1 * x2, 0.0);
