...
class TRendererViewport : public TRenderer
{
public:
    TRendererViewport();

    /** Set format used to print numeric values */
    void set_numeric_format(std::ios_base::fmtflags format,
                                   unsigned int precision);

    /** @alias set_margin_ratio1
        Set amount of 2d margin space for @ref set_window operation
        as ratio of window. */
    void set_margin_ratio(double left, double bottom,
    ...
    void set_margin_output(double left, double bottom,
                           double right, double top);
    ...
    void set_window(const Math::VectorPair2 &window,
                    bool keep_aspect = true);
    void set_window(const Math::Vector2 &center,
                    double radius, bool keep_aspect = true);
...
protected:
    ...
    void set_2d_plot_window(const TPlot &plot);
    /** @internal Draw plot data */
    virtual void draw_plot_data_2d(const TSet1d &data,
                                   const TPlotData &plot);
    /** @internal Draw plot data */
    virtual void draw_plot_data_3d(const TGrid &data,
                                   const TPlotData &plot);
...
private:
    template <int N>
    void draw_axes(const TRendererAxes &a);

    template <int N>
    void draw_axes_tic(const TRendererAxes &a, unsigned int i,
