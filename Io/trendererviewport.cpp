...
void TRendererViewport::set_numeric_format(std::ios_base::fmtflags format,
...
const Math::VectorPair2 & TRendererViewport::get_window() const
{
    return _window2d_fit;
}

double TRendererViewport::x_scale(double x) const
{
    return ((x / (_page[1].x() - _page[0].x())) * _2d_output_res.x());
}

double TRendererViewport::y_scale(double y) const
{
    return ((y / (_page[1].y() - _page[0].y())) * _2d_output_res.y());
}

double TRendererViewport::x_trans_pos(double x) const
{
    return x_scale(x - _page[0].x());
}

double TRendererViewport::y_trans_pos(double y) const
{
    return y_scale(y - _page[0].y());
}

void TRendererViewport::set_margin_output(double width, double height)
{
    set_margin_output(width, height, width, height);
}

void TRendererViewport::set_margin(double width, double height)
{
    set_margin(width, height, width, height);
}

void TRendererViewport::set_margin_ratio(double width, double height)
{
    set_margin_ratio(width, height, width, height);
}

TRendererViewport::TRendererViewport()
...
void TRendererViewport::set_2d_size(double width, double height)
{
    _2d_output_res = Math::Vector2(width, height);
}

void TRendererViewport::set_window(const Math::Vector2 &center,
...
void TRendererViewport::update_2d_window()
{
}
...
void TRendererViewport::set_window(const Math::VectorPair2 &window, bool keep_aspect)
{
...
void TRendererViewport::set_page_layout(unsigned int cols, unsigned int rows)
{
    _cols = cols;
    _rows = rows;
    set_page(0);
...
void TRendererViewport::set_page(unsigned int page)
{
    if (page >= _cols * _rows)
...
void TRendererViewport::draw_pages_grid(const TRgb &rgb)
{
    Math::Vector2     size(_window2d[1].x() - _window2d[0].x(),
            _window2d[1].y() - _window2d[0].y());
...
void TRendererViewport::set_margin(double left, double bottom, double right, double top)
{
...
void TRendererViewport::set_margin_ratio(double left, double bottom,
...
void TRendererViewport::set_margin_output(double left, double bottom,
                                          double right, double top)
{
...
void TRendererViewport::set_camera_direction(const Math::Vector3 &dir)
{
...
void TRendererViewport::set_camera_position(const Math::Vector3 &pos)
{
    Math::Transform<3> t(get_camera_transform());
...
void TRendererViewport::clear()
{
}

void TRendererViewport::flush()
{
}
...
void TRendererViewport::set_2d_plot_window(const TPlot &plot)
{
...
void TRendererViewport::draw_plot(const TPlot &plot)
{
    ...
    case 1:
        ...
        {
            const TPlotData &d = plot.get_plot_data(i);
            draw_plot_data_2d(static_cast<const TSet1d&>(d.get_set()), d);
        }
...
void TRendererViewport::draw_frame_2d()
{
    Math::Vector2 fr[4];
...
void TRendererViewport::draw_plot_data_2d(const TSet1d &data, const TPlotData &style)
{
...
void TRendererViewport::draw_plot_data_3d(const TGrid &data, const TPlotData &style)
{
    ...
    if (style.get_style() & Data::InterpolatePlot)
    {
        ...
        for (double x = xr.first; x <= xr.second; x += xs)
            for (double y = yr.first; y <= yr.second; y += ys)
            {
                ...
                draw_point(p, TRgb(0, 1, 0), PointStyleCross);
            }
    }
...
#endif
                draw_triangle(Math::Triangle<3>(p11, p12, p22), Math::Triangle<3>(g11, g12, g22), TRgb(0, 1, 0));
                draw_triangle(Math::Triangle<3>(p22, p21, p11), Math::Triangle<3>(g22, g21, g11), TRgb(0, 1, 0));
            }
    }
    ...
    if (style.get_style() & Data::LinePlot)
    {
        ...
        for (x = 0; x < data.get_count(0); x++)
        {
            ...
            for (y = 1; y < data.get_count(1); y++)
            {
                ...
                draw_segment(Math::VectorPair3(p1, p2), TRgb(1, 1, 1));
    ...
    if (style.get_style() & Data::PointPlot)
    {
        for (unsigned int x = 0; x < data.get_count(0); x++)
            for (unsigned int y = 0; y < data.get_count(1); y++)
            {
                ...
                draw_point(p, TRgb(1, 0, 1), PointStyleCross);
            }
    }
}

void TRendererViewport::draw_axes_2d(const TRendererAxes &a)
{
    draw_axes<2>(a);
}

void TRendererViewport::draw_axes_3d(const TRendererAxes &a)
{
    //      draw_axes<3>(a, range);
}

template <int N>
void TRendererViewport::draw_axes(const TRendererAxes &a)
{
    ...
    {
        const TRendererAxes::Axis &ax = a._axes[i];
...
template <int N>
void TRendererViewport::draw_axes_tic(const TRendererAxes &a, unsigned int i,
