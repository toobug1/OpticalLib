...
const TDistribution TZernike::default_dist(Trace::HexaPolarDist, 10);
...
zp_t * const zp[TZernike::term_count] =
{
    &zp_0,
    &zp_1, &zp_2, &zp_3,
...
zp_d_t * const zp_d[TZernike::term_count] =
{
    0,
...
TZernike::TZernike(double radius, double unit_scale)
    : _scale(unit_scale),
      _threshold(1e-10),
      _radius(radius),
      _enabled_count(0)
...
TZernike::TZernike(double radius, double coefs[],
...
double TZernike::sagitta(const Math::Vector2 & xy) const
{
...
void TZernike::derivative(const Math::Vector2 & xy, Math::Vector2 & dxdy) const
{
    struct zp_precalc_s p(xy.x() / _radius, xy.y() / _radius);
...
double TZernike::fit(const TCurveBase &curve, const TDistribution & d)
{
...
void TZernike::update_threshold_state()
{
...
void TZernike::set_term_state(unsigned int n, bool enabled)
{
    unsigned int i;
...
bool TZernike::get_term_state(unsigned int n)
{
    for (unsigned int i = 0; i < _enabled_count; i++)
        if (_enabled_list[i] == n)
            return true;
...
double TZernike::zernike_poly(unsigned int n, const Math::Vector2 & xy)
{
...
void TZernike::zernike_poly_d(unsigned int n, const Math::Vector2 & xy, Math::Vector2 & dxdy)
{
    struct zp_precalc_s p(xy.x(), xy.y());
...
void TZernike::set_radius(double radius)
{
    _radius = radius;
}

double TZernike::get_radius() const
{
    return _radius;
}

void TZernike::set_coefficient(unsigned int n, double c)
{
    assert(n < term_count);
    _coeff[n] = c;
...
double TZernike::get_coefficient(unsigned int n) const
{
    assert(n < term_count);
    return _coeff[n];
}

void TZernike::set_coefficients_threshold(double t)
{
    _threshold = t;
    update_threshold_state();
}

void TZernike::set_coefficients_scale(double s)
{
    _scale = s;
}
